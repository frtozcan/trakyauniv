...
with CustomObjectScope({'loss':"loss",'acc':"accuracy", 'dice_coef': dice_coef,'iou': iou,'recall':"recall",'precision':"precision"}):
    model = tf.keras.models.load_model(yol + "fognet20210912.hdf5")


print(len(model.layers))

path = 'E:/DATASETLER/our_data/test/img/'
...
start1 = datetime.now() 
for filename in filename_list:#[11:12]:
    ...
    cv2.imwrite("./MODELLER/"+MODEL_NAME+"/files/result/"+ name +"list_mask.png", gt)
    cv2.imwrite("./MODELLER/"+MODEL_NAME+"/files/result/"+ name +"list_pred.jpg", predicted)
    cv2.imwrite("./MODELLER/"+MODEL_NAME+"/files/result/"+ name +"list_orj.jpg", image1)
    cv2.imwrite("./MODELLER/"+MODEL_NAME+"/files/result/"+ name +"list_gather.jpg", sonuc)
    """
    ...
    print(f"Dice_Coeff(F1 Score): {f1_value:0.5f}")
...
df = pd.DataFrame(SCORE, columns = ["Image Name", "Acc", "Dice_Coeff(F1 Score)", "Jaccard", "Recall", "Precision"])
df.to_csv("files/CHAOS_score.csv")        
